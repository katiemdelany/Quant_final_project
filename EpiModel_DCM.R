#Tested using Influenza H1N1 data from Outbreaks package
## ----setup, include = FALSE----------------------------------------------
require(knitr)
require(EpiModel)
...
init <- init.dcm(s.num = 763, i.num = 1)
...
## ----dcmSir--------------------------------------------------------------
param <- param.dcm(inf.prob = 0.4, act.rate = 1, rec.rate = 1/20,
                   a.rate = 1/100, ds.rate = 1/100, di.rate = 1/80, dr.rate = 1/100)
init <- init.dcm(s.num = 763, i.num = 1, r.num = 0)
...
## ----dcmSis, results='hide'----------------------------------------------
param <- param.dcm(inf.prob = 0.4, act.rate = seq(0.25, 0.5, 0.05), rec.rate = 0.02)
init <- init.dcm(s.num = 763, i.num = 1)
...
## ----dcmSisSensOpts------------------------------------------------------
act.rates <- c(0.2, 0.2, 0.3, 0.3, 0.4, 0.4)
...
param <- param.dcm(inf.prob = 0.3,  inf.prob.g2 = 0.2, act.rate = 0.25, balance = "g1",
                   a.rate = 1/100, a.rate.g2 = NA, ds.rate = 1/100, ds.rate.g2 = 1/100,
                   di.rate = 1/50, di.rate.g2 = 1/50)
init <- init.dcm(s.num = 763, i.num = 1, s.num.g2 = 500, i.num.g2 = 0)
control <- control.dcm(type = "SI", nsteps = 500)
mod <- dcm(param, init, control)
