Our aim is to characterize chlamydia diagnosis rates by age, gender and geographically and for that purpose we employ a Poisson regression model.
...
```{r load_data_years, echo = FALSE }
	df <- as.data.table(read.csv("Chlamydia_2.csv", stringsAsFactors=FALSE))
	df[, LOG_POP:= log(POP)]
	df[, MALE:= as.integer(GENDER=='Male')]
	#	overall Gono incidence for male and female across England
	dg2	<- df[, list(CASES=sum(CASES), PERSON_YEARS=sum(POP)), by=c('GENDER','PHE','AGE')]	
```
...
```{r plot echo = FALSE}	
	ggplot(dg2) +
			geom_line(aes(x=AGE, colour=PHE, group=PHE, y=PERSON_YEARS)) +
			theme_bw() +
			theme(axis.text.x = element_text(angle = 90, hjust = 1), legend.position='bottom') +	
			facet_grid(GENDER~PHE, scales='free') +
...
```{r plot_diag_rates_postmedian, echo = FALSE}	
	...
	ggplot(dg2) +
			...
			geom_point(aes(y=PHE, x= CASES/PERSON_YEARS*1e5)) +
			theme_bw() +			
			labs(x='\ncrude Chlamydia incidence\nper 100,000', y='PHE region') +
...
```{r fit2 dependson = c("load_data_years"), echo = FALSE}	
	dg2[, PHE2:= as.integer(factor(PHE))]
	mg2.2	<- map2stan(
			alist(
					CASES ~ dpois(kappa),
					log(kappa) <- alpha + alpha_phe[PHE2] + LOG_PERSON_YEARS + bmale*MALE,					
					alpha <- dnorm(0,100),
					alpha_phe[PHE2] <- dnorm(0,pop_alpha_sig),
					bmale <- dnorm(0,10),
					pop_alpha_sig <- dcauchy(0,2)
			),
			data=as.data.frame(dg2), 
			start=list(alpha=-8, alpha_phe=rep(0,2), bmale=0, pop_alpha_sig=1),
			iter=1e4, warmup=5e2)
...
```{r ir_gender_PHEC, echo = FALSE}
  post	<- extract.samples(mg2.2)
	...
	tmp	<- matrix(data=post$alpha, ncol=2, nrow=length(post$alpha)) + post$alpha_phe	
	...
	mg2.2.inc.rate.female.log[,PHE2:= 1:2]		
	mg2.2.inc.rate.female.log[,MALE:= 0]
	
	#	Incidence per 100,000 men for each PHEC
	tmp	<- matrix(data=post$alpha, ncol=2, nrow=length(post$alpha)) + post$alpha_phe + matrix(data=post$bmale, ncol=2, nrow=length(post$bmale))	
	...
	mg2.2.inc.rate.male.log[,PHE2:= 1:2]		
	mg2.2.inc.rate.male.log[,MALE:= 1]
	
	#	put this together
	mg2.2.inc.rate.log	<- rbind(mg2.2.inc.rate.female.log, mg2.2.inc.rate.male.log)
	mg2.2.inc.rate.log	<- merge(mg2.2.inc.rate.log, unique(subset(dg2, select=c(PHE, PHE2))), by='PHE2')	
	
	#Plot region-specific incidence estimates with two panels for males and females against the crude estimates
	ggplot(dg2) +
			geom_point(aes(y=PHE, x= CASES/PERSON_YEARS*1e5), colour='grey60') +
			geom_point(aes(y=PHE, x=MEDIAN), data=mg2.2.inc.rate.log) +
			geom_errorbarh(aes(y=PHE, xmin=CL, xmax=CU, x=MEDIAN), data=mg2.2.inc.rate.log, height=0.3) +
			theme_bw() +			
			labs(x='\ncrude Chlamydia incidence\nper 100,000', y='PHE region') +
			facet_grid(~factor(MALE, levels=c(0,1), labels=c('female','male')), scales='free')
```	
...
```{r plot_diag_rates_age, echo = FALSE}
	ggplot(dg2, aes(x=AGE, colour=PHE, group=PHE, y=CASES/PERSON_YEARS*1e5)) +
			geom_line() +
			theme_bw() +
...
```{r fit3 dependson = c("load_data_years"), echo = FALSE}
  dg2[, PHE2:= as.integer(factor(PHE))]
	...
	dg2[, LONDON:= as.integer(PHE=='London')]
	mg2.4	<- map2stan(
			alist(
					CASES ~ dpois(kappa),
					log(kappa) <- alpha + alpha_phec[PHE2] + age_male[AGE2]*MALE + age_female[AGE2]*(1-MALE) + 
							LOG_PERSON_YEARS + bmale*MALE + bmale_london*MALE*LONDON, 
					alpha <- dnorm(0,100),
					alpha_phec[PHE2] <- dnorm(0,pop_alpha_sig),
			...
			data=as.data.frame(dg2), 
			start=list(	alpha=-8, alpha_phec=rep(0,2), age_male=rep(0,4), age_female=rep(0,4), 
...
```{r predictive_check_mix ,echo = FALSE}
	...
	ggplot(y.PI, aes(x=AGE, ymin=predicted_cases_l95, ymax=predicted_cases_u95)) +
			...
			geom_point(aes(y=CASES), colour='red') +
			geom_errorbar() +
			theme_bw() +			
			theme(axis.text.x = element_text(angle = 90, hjust = 1), legend.position='bottom') +
			...
			facet_wrap(GENDER~PHE, scales='free', ncol=9)
```
...
```{r plot_inc_dummy, echo = FALSE}
	...
	tmp		<- dg2[, 	{
				z	<- dummy(PHE2,AGE2,MALE)
				list(STAT=c('MEDIAN','CL','CU'), VALUE=z)
			}, by=c('PHE','PHE2','AGE','AGE2','GENDER','MALE')]
	tmp		<- dcast.data.table(tmp, PHE+PHE2+AGE+AGE2+GENDER+MALE~STAT, value.var='VALUE')			
	ggplot(dg2) +
			geom_line(aes(x=AGE, colour=PHE, group=PHE, y=CASES/PERSON_YEARS*1e5)) +
